<template>
	<div ref="iphoneScreen" class="iphone position-relative" :style="{ transform: `scale(${scale})`, width: `${width + 48}px`, height: `${height + 200}px` }">
		<slot name="head">
			<img class="position-absolute zindex-109" style="width: 372px; top: 90px; left: 20px; height: 20px" :src="require(`@/assets/images/head-stausBar1.png`)" v-if="headStausBar"/>
		</slot>
		...
		<div class="iphone-screen position-relative" :style="{ backgroundColor: backgroundColor, width: `${width}px`, height: `${height}px` }" @scroll="onScroll">
			<div class="pt-20 w-100 h-100 position-absolute left-0 right-0" v-if="headStausBar"><slot name="screen"></slot></div>
			<slot name="screen" v-else></slot>
		</div>
		<div class="buttons">
			<span class="on-off" />
			<span class="sleep" />
...
.iphone-screen {
	...
	border-radius: 3px;
	// iphone-screen的滚动是 嵌入的iframe的滚动而不是其iframe的父盒子滚动
	overflow: hidden;	//这里不加overflow的话会导致 iframe与其父盒子存有间隙
	// overflow-y: scroll;

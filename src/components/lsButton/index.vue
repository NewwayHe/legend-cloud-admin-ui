...
<template>
    ...
    >
        <slot>
            <span v-if="text" :class="[`v-${mode}`]">{{ text }}</span>
        </slot>
    </el-button>
</template>
<script>
...
export default {
    ...
    computed: {
        ...
        optionstyle() {
            const style = {
                ...
                border:
                    this.options && this.options.bdSet
                        ? `${this.options.bdSet}`
                        : this.options && this.options.bd
                        ? `1px solid ${this.options.bd}`
                        : '', // 处理border线
    ...
    watch: {
        ...
        disabledData(newVal, oldValue) {
            // console.log('ccccccccccccccdisabledData',newVal,oldValue)
            if (oldValue) {

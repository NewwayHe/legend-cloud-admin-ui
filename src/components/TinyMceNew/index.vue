<template>
    <div :class="['tinymce-container', { fullscreen: fullscreen }]">
        <vue-tinymce v-model="content" :setting="setting" :setup="setupCb" @change="valChange" />
        <div class="editor-custom-btn-container">
            <!-- <editorImage color="#1890ff" class="editor-upload-btn" @successCBK="imageSuccessCBK" /> -->
            <imgCenter :limit="15" :is-show-selected="false" button mult in-rich-text @input="imageSuccessCBK"></imgCenter>
        </div>
    </div>
</template>
...
export default {
    ...
    props: {
        ...
        plugins: {
            type: String,
            default:
                'advlist anchor autolink autosave code codesample colorpicker colorpicker contextmenu directionality emoticons fullscreen hr image imagetools insertdatetime link lists media nonbreaking noneditable pagebreak paste preview print save searchreplace  tabfocus table template textcolor textpattern visualblocks visualchars wordcount'
        }
    },
    data() {
        return {
    ...
    computed: {
        ...
        setting({ width, height, toolbar, menubar, plugins }) {
            return {
                ...
                extended_valid_elements: 'video[width|height|src|style|poster|autoplay|controls|muted]',
                ...
                // media_url_resolver: this.videoResolver
            }
        }
    },
    watch: {
    ...
    methods: {
        ...
        videoResolver(data, resolve, reject) {
            console.log('data--', data) //URL
            // const html = `<video width="${width}" height="${height}" poster="${poster}" controls
            ...
            //     </video>
            // `
            resolve({ html: '<span>hhh</span>' })
        },
        videoCallback(data) {
            //这个只影响到了外面的 非富文本里的 富文本里的不能自动播放   controls="controls" autoplay="autoplay"
        ...
        imageSuccessCBK(arr) {
            ...
            arr.forEach((src) => {
                ...
                if (this.diffPicOrVideo(`.${picExt}`)) {
                    picStr += `<video src="${that.$photoServer + src}" data-mce-src="${
                        that.$photoServer + src
                    }" controls autoplay muted width="300" height="150" style="max-width: 100%; background-color: #000; vertical-align: bottom;"></video>`
                } else {
                    picStr += `<img src="${that.$photoServer + src}" data-mce-src="${
                        that.$photoServer + src

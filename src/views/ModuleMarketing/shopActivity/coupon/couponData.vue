...
<template>
    <section class="redData">
        ...
        <el-card>
            ...
            <p>{{ decodeURIComponent($route.query.couponName) }}</p>
            <div class="mt-20">
                <el-select size="small" v-model="activityModel.origin" placeholder="请选择">
                    <el-option v-for="opt in originOpts" :key="opt.value"
                    :label="opt.label" :value="opt.value">
                    </el-option>
                </el-select>
                <el-date-picker class="mx-20" size="small" v-model="activityModel.date" type="daterange" range-separator="至"
            ...
            <el-row :gutter="20" class="mt-20">
                    ...
                    <p>
                        ...
                        <el-tooltip effect="dark" placement="right" popper-class="redData-questionPop" trigger="hover">
                            <div slot="content">
                                ...
                                <p>营销支付金额 = 统计时间段内，已付款的活动订单中所有参与活动的商品使用原价和活动价的差额 =（销售价 - 活动价）* 商品数量的累加之和；（也就是参与活动的商品用正常销售价销售和参与活动销售之间的差额）；</p>
                                <p>营销利润金额 = 统计时间段内，已付款的活动订单中所有参与活动的商品的利润 =（活动价 - 成本价）* 商品数量累加之和；</p>
                            </div>
                ...
                <el-col :span="12">
                    <p>
                        ...
                        <el-tooltip effect="dark" placement="right" popper-class="redData-questionPop" trigger="hover">
                            <div slot="content">
                                <p>成交金额 = 统计时间段内，已付款并变为已完成状态的订单的金额（若订单完成了售后需要扣除对应金额，不统计运费金额）； </p> 
        ...
        <el-card class="mt-20">
            <div>
                <el-select size="small" v-model="recruitModel.origin" placeholder="请选择">
                    <el-option v-for="opt in originOpts" :key="opt.value"
                    :label="opt.label" :value="opt.value">
                    </el-option>
                </el-select>
                <el-date-picker class="mx-20" size="small" v-model="recruitModel.date" type="daterange" range-separator="至"
                    value-format="yyyy-MM-dd" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
            ...
            <el-row :gutter="20" type="flex" justify="space-between" class="mt-20">
                <el-col :span="12">
                    <p>
                        ...
                        <el-tooltip effect="dark" placement="right" popper-class="redData-questionPop" trigger="hover">
                            <div slot="content">
                                <p>下单用户 = 统计参与该活动的所有用户ID的数量，一个用户ID不管通过什么渠道提交了订单，一个用户ID标记为一个用户数；若一个用户生成多条订单数据，需要去重；  </p> 
                                <p>下单新用户 = 统计参与该活动的未在该店铺生成订单的用户ID的数量（若已生成订单但是已完成了售后的用户，也标记为新用户）； </p>
                    ...
                    <div class="mt-10 d-flex a-center">
                        <UserPie :chartData="userPieData" width="70%" height="300px"/>
                        <div class="font-12">
                            <p>下单用户{{recruitData.userCount||0}}人</p>
                            <p class="my-10">下单新用户{{recruitData.orderNewUserCount || 0}}人， 占比：{{getPercentage(recruitData.orderNewUserCount,recruitData.userCount)}}% </p>
                            <p>下单旧用户{{recruitData.orderOldUserCount || 0}}人，占比：{{getPercentage(recruitData.orderOldUserCount,recruitData.userCount)}}%</p>
                        </div>
                    </div>
                </el-col>
                    ...
                    <p>
                        ...
                        <el-tooltip effect="dark" placement="right" popper-class="redData-questionPop" trigger="hover">
                            <div slot="content">
                                ...
                                <p>下单用户数 = 统计时间段内，使用优惠券并且已提交的订单的用户数量，一个人用户ID通过多个渠道访问多次时需要去重，至统计一次；即一个用户ID在一个人活动中统计的用户数最多只有一次； </p>
                                <p>成交订单数 = 统计时间段内，使用优惠券并且已付款并变为已完成状态的订单的数量（若订单完成了售后需要扣除对应数量）； </p>
                                <p>成交用户数 = 统计时间段内，使用优惠券并且已付款并变为已完成状态的订单的用户数量，一个人用户ID通过多个渠道访问多次时需要去重，至统计一次；即一个用户ID在一个人活动中统计的用户数最多只有一次； </p>
                                <p>转化率 = 成交用户数 ÷ 领取用户数；</p>
                            </div>
                            <i class="el-icon-question text-main font-16 ml-5"></i>
                        </el-tooltip>
                    ...
                    <div class="mt-10 convert-wrap flex-1 flex-between w-100">
                        <div class="position-relative overflow-h convert-container" style="width: 80%;">
                            <div class="convert-item">
                                ...
                                <div class="convert-inner">访客<span></span></div>
                            </div>
                            <div class="convert-item">
                                <div class="convert-inner">
                                    <span>下单订单数</span>
                                ...
                                <div class="convert-inner">下单<span></span></div>
                            </div>
                            <div class="convert-item">
                                <div class="convert-inner">
                                    <span>成交订单数</span>
                                ...
                                <div class="convert-inner">成交<span></span></div>
                            </div>
                        </div>
        ...
        <el-card class="mt-20">
            <p>
                ...
                <el-tooltip effect="dark" placement="right" popper-class="redData-questionPop" trigger="hover">
                    <div slot="content">
                        <p>新增成交金额 = 显示当天00:00:00至23:59:59，已付款并变为已完成状态的订单的金额（若订单完成了售后需要扣除对应金额，不统计运费金额）；大于等于0的整数，保留小数点后两位；</p> 
                        <p>累计成交金额 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，已付款并变为已完成状态的订单的金额（若订单完成了售后需要扣除对应金额，不统计运费金额）；大于等于0的整数，保留小数点后两位；</p>
                        <p>新增成交订单数 = 显示当天00:00:00至23:59:59，已付款并变为已完成状态的订单的数量（若订单完成了售后需要扣除对应数量）；大于等于0的整数；</p>
                        <p>累计成交订单数 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，已付款并变为已完成状态的订单的数量（若订单完成了售后需要扣除对应数量）；大于等于0的整数；</p>
                        <p>新增成交数量 = 显示当天00:00:00至23:59:59，已付款并变为已完成状态的订单中的SKU的数量（若商品完成了售后需要扣除对应数量）；大于等于0的整数；</p>
                        <p>累计成交数量 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，已付款并变为已完成状态的订单中的SKU的数量（若商品完成了售后需要扣除对应数量）；大于等于0的整数；</p>
                        <p>新增下单用户数 = 显示当天00:00:00至23:59:59， 统计时间段内，已提交的订单的用户数量，一个人用户ID通过多个渠道访问多次时需要去重，至统计一次；即一个用户ID在一个人活动中统计的人数最多只有一次；</p>
                        <p>累计下单用户数 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，统计时间段内，已提交的订单的用户数量，一个人用户ID通过多个渠道访问多次时需要去重，至统计一次；即一个用户ID在一个人活动中统计的人数最多只有一次；</p>
                        <p>新增访问次数 = 显示当天00:00:00至23:59:59， 访问参与活动的商品详情页的次数，按照接口请求数据为准，一天内同一个用户ID访问多次只统计为一次，但是同一个用户ID分别以多个渠道访问多次则每个渠道统计为一次；</p>
                        <p>累计访问次数 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，访问参与活动的商品详情页的次数，按照接口请求数据为准，一天内同一个用户ID访问多次只统计为一次，但是同一个用户ID分别以多个渠道访问多次则每个渠道统计为一次；</p>
                        <p>新增访问人数 = 显示当天00:00:00至23:59:59，访问参与活动商品的商品详情页的用户ID数量；一天内同一个用户ID通过多个渠道访问多次只统计为一人；</p>
                        <p>累计访问人数 = 累计时间段内的开始日期00:00:00至后面每一天的23:59:59，访问参与活动商品的商品详情页的用户ID数量；一天内同一个用户ID通过多个渠道访问多次只统计为一人；</p>
                    </div>
                    <i class="el-icon-question text-main font-16 ml-5"></i>
                </el-tooltip>
            </p>
            <div class="mt-20">
                <el-select size="small" v-model="trendModel.origin" placeholder="请选择">
                    <el-option v-for="opt in originOpts" :key="opt.value"
                    :label="opt.label" :value="opt.value">
                    </el-option>
                </el-select>
                <el-date-picker class="mx-20" size="small" v-model="trendModel.date" type="daterange" range-separator="至"
                    value-format="yyyy-MM-dd" start-placeholder="开始日期" end-placeholder="结束日期">
                </el-date-picker>
            ...
            <div class="mt-20">
                <TrendLine :chartData="trendLineData" width="100%" height="600px" :xAxis="trendModel.date"/>
            </div>
        </el-card>
    </section>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            trendLineData: [    //数据趋势线性图数据 [搜索日期有几天 value就有多长]
    ...
    methods: {
        ...
        // 请求数据处理
        getDataCount(type){
            let {origin:source,date:[beginTime,endTime]} = this[type];
        ...
        recruitModelCount(){
            cpApi.userAmountCount(this.getDataCount('recruitModel')).then(res=>{
                if(!this.$utils.test.isEmpty(res)){
            ...
            cpApi.userDataCount(this.getDataCount('trendModel')).then(res=>{
                if(res.data&&res.data.length){
                     this.trendLineData.map(item=>{
                        this.$set(item, 'value', res.data.map(resMap=>{return resMap[item.type] || 0}))
                    })
                }
            })
        },

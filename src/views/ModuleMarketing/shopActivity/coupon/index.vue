        ...
        <el-card shadow :body-style="{padding:`20px 20px 10px 20px`}">
            ...
            <div class="table">
				...
				<el-table ref="multipleTable" v-loading="tableListLoading" :data="tableList" tooltip-effect="dark" class="w-100" header-row-class-name="headerRow" row-key="id" @selection-change="selectionChange">
					...
					<el-table-column type="selection" reserve-selection width="42" :selectable="(row)=>{return !(row.status===0||row.status==1||row.status==2)}"/>
                    ...
                    <el-table-column prop="status" label="状态">
                        <template slot-scope="scope">
                            <span :class="scope.row.status == -2 ? 'status-veto' : scope.row.status == 1 ? 'status-wait' : 'status-done'">{{ scope.row.status | getLabelText() }}</span>
                        </template>
                    </el-table-column>
                    <el-table-column label="操作" fixed="right" width="180" >
                        <template slot-scope="scope">
                            <span class="table__action flex-center">
                                ...
                                <el-link :underline="false" type="primary" @click.stop="$router.push({ name: 'couponData', query: { 'couponId' : scope.row.id, 'couponName': encodeURIComponent(scope.row.title) }})">数据</el-link>
                                <el-link

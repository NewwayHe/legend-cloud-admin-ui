<template>
	<div class="customTable__warp" ref="wrapTable">
								...
								<td colspan="3">
									<div class="d-flex a-center line-h-md" v-if="item.orderItemDTOList.length == 1">
										...
										<div class="ml-10 flex-col-between">
											...
											<span class="text-999 mt-10 text-left" v-if="item.productAttribute">{{ item.productAttribute }}</span>
										</div>
									</div>
									<div
										v-else
									...
									>
										<div class="d-flex a-center line-h-md position-relative overflow-h" :ref="`swiper-container-${index}`">
											<el-popover
												placement="top-start"
												width="500"
												trigger="hover"
		...
		<el-dialog title="审核" custom-class="dialog-form-small" :visible.sync="isVisible">
			...
			<span slot="footer">
				<el-button size="small" @click="isVisible = false" :disabled="isExaming">取 消</el-button>
				<el-button type="primary" size="small" @click="examineSubmit" :disabled="isExaming">确 定</el-button>
			</span>
		</el-dialog>

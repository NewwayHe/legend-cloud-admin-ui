<template>
	<div>
						...
						<table v-for="(item, indexs) in tableList" :key="indexs" class="customTable__table">
							...
							<tbody>
								<tr class="customTable__trHead">
									<td>
										<el-checkbox :label="item" :disabled="item.applyStatus != 2" />
									</td>
								...
								<tr v-for="(goodsInfo, index) in item.orderItemDTOList" :key="goodsInfo.id">
									...
									<td colspan="3">
										<div class="d-flex a-center line-h-md">
											...
											<div class="ml-10 flex-col-between">
												...
												<el-link :underline="false" type="primary" @click="proPreview(goodsInfo)" style="font-size: 0">
									...
									<!-- 操作账号/时间 -->
									<td v-if="!index" :rowspan="item.orderItemDTOList.length">
										<template v-if="item.adminOperator || item.adminOperatorTime">
											<div class="word-breakall">{{ item.adminOperator || '-' }}</div>
											<div>{{ item.adminOperatorTime || '-' }}</div>
										</template>
										<span v-else>-</span>
									</td>
									<!-- 取消原因 -->
									...
									<!-- 备注说明 -->
									<td colspan="2" v-if="!index" :rowspan="item.orderItemDTOList.length">
		...
		<auditOrderApplyCancelDialog :orderItemList="orderItemList" ref="auditOrderApplyCancelDialog" @finish="finish"></auditOrderApplyCancelDialog>
	</div>
</template>
<script>
...
import auditOrderApplyCancelDialog from './auditOrderApplyCancelDialog.vue'
export default {
	mixins: [cud],
	components: {
		dialogPreview,
	...
	methods: {
		...
		batchExamine() {
			this.orderItemList = this.checkList
			this.$refs.auditOrderApplyCancelDialog.showDialog()
		},
		// 父组件点击全选
		selectedAllOrNot() {
			if (this.checkList.length) {
				...
				this.checkList = this.tableList.filter((item) => {
		...
		getData() {
			this.$emit('operate', { success: true })
		},
		// 去订单详情
		toDetail(order, name) {

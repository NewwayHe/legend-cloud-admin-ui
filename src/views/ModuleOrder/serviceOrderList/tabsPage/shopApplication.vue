			...
			<el-form :inline="true" :model="searchFilters" size="small" ref="formWrapBtn">
				...
				<el-form-item label="取消原因">
					<el-select v-model="searchFilters.reason">
						<el-option :label="item.value" :value="item.value" v-for="item in reasonList" :key="item.id"></el-option>
					</el-select>
				</el-form-item>
...
import { getExcel } from '@/mixins/pages/formFn'
...
export default {
	...
	data() {
		return {
			...
			reasonList: [],
			url: {
				getData: '/order/admin/order/refund/pageCancelOrder',
				getExcel: '/order/admin/order/refund/excel/cancel'
			},
			isMounted: true
		}
	},

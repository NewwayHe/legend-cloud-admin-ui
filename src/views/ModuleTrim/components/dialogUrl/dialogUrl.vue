...
*/ -->
	...
	<div class="cursor-pointer" :class="[className, hover ? '' : 'w-100 flex-start overflow-h']" @click="showDialog()">
		...
		<template v-if="hover">
			<el-popover placement="top" trigger="hover" v-if="value[field] && value[field].id">
				<div class="flex-start overflow-h cursor-pointer hover-scale1-1" style="max-width: 400px" @click="showDialog()">
					...
					<div class="ml-10 flex-1 flex-start overflow-h">
						<ls-image :src="value[field].pic" :isPreview="false" :options="{ w: 24, h: 24 }" v-if="value[field].pic" />
						<span v-if="value[field].name" class="line-clamp1 ml-5">{{ value[field].name.toString() }}</span>
					</div>
				</div>
				<i class="iconfont icon-lianjie text-main main-hover" style="font-size: 24px" slot="reference" />
			</el-popover>
			<i class="iconfont icon-lianjie text-main main-hover" style="font-size: 24px" v-else />
		</template>
		<!-- 如是是直接在页面上显示内容 -->
		<template v-else>

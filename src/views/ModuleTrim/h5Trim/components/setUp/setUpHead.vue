    ...
    */ -->
    <div>
		<div
			...
			:style="{height:`${(platform != 'H5'?20:0) + 44 + (value.head.title.name?paddingBottom:0)  + (floors.type == 'searchAndTab'&&searchTabData.tabsShow?40:0) }px`}"
        ...
        <template v-if="value.category == 'INDEX'||value.category == 'INDEX-T'">
            ...
            >
				<div class="h-100 w-100 content-box" :style="{ color: floors.color || '#ffffff',background: background,paddingBottom:floors.title.name?`${paddingBottom}px`:'' }">
					<div class="flex-between h-100 w-100 px-10">
					    <div class="mr-10" v-if="floors.locationAndLogo && floors.locationAndLogo.flag == 1">
					        广东
					        <i class="el-icon-arrow-down v-bottom ml-5" />
					    </div>
					    <div class="mr-10" style="height: 28px" v-if="floors.locationAndLogo && floors.locationAndLogo.flag == 2">
					        <ls-image :src="floors.locationAndLogo.logo" :options="{h: '100%', w: floors.locationAndLogo.logoWidth, br:floors.locationAndLogo.logoBorderRadius}" :isPreview="false"></ls-image>
					    </div>
						<div class="flex-1 h-100 position-relative flex-start" ref="midCon">
							<div class="pl-10 line-clamp1" :style="{fontSize: floors.title.size+'px',opacity:midConOpacity,fontWeight: floors.title.fontWeight ? 'bold' : 'unset',}" v-if="floors.title.name">{{floors.title.name}}</div>
							<div class="rounded-15 text-bbb px-10 flex-start zindex-2" 
								...
								:style="{backgroundColor:floors.seachColor?floors.seachColor:(floors.backgroundColor=='#FFFFFF'||!floors.backgroundColor)?'#f6f7f8':'#ffffff',
									left:floors.title.name?`${ -(midConInfo.offsetLeft-10) + (midConInfo.offsetLeft-10)*(1-midConOpacity)}px`:'',
									bottom:floors.title.name?`${ -30 + 38*(1-midConOpacity)}px`:'',
									width:floors.title.name?(midConInfo.offsetWidth?`${midConInfo.offsetWidth + (355-midConInfo.offsetWidth)*midConOpacity}px`:'355px'):''
								}"
							>
							<!-- '-50px':'', -->
				...
				<div v-if="floors.type == 'searchAndTab'" class="w-100 position-relative">
				    <div v-if="searchTabData.tabsShow" 
					...
					>
				            ...
				            >
				                <span
				                    ...
				                    :style="{
				                        ...
				                        borderBottom: `${item.isSelect && searchTabData.tabsType == 'line' ? 2 : 0}px solid ${item.isSelect ? curTabColor : ''}`
				                    }"
				                >
				                    {{ item.title }}
				                </span>
						...
						<div class="flex-end ml-10" v-if="searchTabData.showCategoryIcon||searchTabData.showCategoryText">
							<div class="iconfont icon-fenlei4 mr-5" v-if="searchTabData.showCategoryIcon"></div>
							<div class="mr-5" v-if="searchTabData.showCategoryText">分类</div>
						</div>
				    </div>
				</div>
            </div>
...
export default {
    ...
    computed: {
		...
		background(){
			return this.floors.themeColor? (this.floors.bgGradualChange ? this.themesRGBA : this.value.themeColor) :( this.floors.bgGradualChange ? this.themesRGBA : this.backgroundColor)
		},
		searchTabData() {
		...
		tabThemesRGBA() {
			let tempColor = this.searchTabData?.tabBgThemeColor ? this.value.themeColor : this.$utils.color.getHexColor(this.searchTabData?.tabBgColor)
			const color = this.$utils.color.colorRgb(tempColor)
			return tempColor?`rgba(${color.r}, ${color.g},${color.b},${this.tabOpacity})`:''
		},
		tabBackground(){
			...
			return themeColor? (bgGradualChange ? this.tabThemesRGBA : this.value.themeColor) :( bgGradualChange ? this.tabThemesRGBA : this.searchTabData?.tabBgColor)
		},
		// 如果是后台选择了[搜索+轮播图]且开了轮播图开关且已经上传了轮播图
		showBanner(){
			return this.value.head && this.value.head.type == 'searchAndBanner' && this.value.head.data.searchAndBannerData.showBanner&&this.value.head.data.searchAndBannerData.banner.imgList.length
		},
		paddingBottom(){
					...
					this.$nextTick(() => {
						...
						if(tabList&&tabList.length){
							...
							if(this.value.head.data.searchAndTabData.linkType == 'poster') {
								eventBus.$emit('iframeUrl', id?`${this.$config.shareUrl}/ModuleCommon/poster/poster?templateId=${id}&device=mobile`:'')
							}
							
							// 让选中的元素居中
							if(this.$refs['tabListItem'+index]&&this.$refs['tabListItem'+index][0]&&this.$refs['tabListItem'+index][0].scrollIntoView()){
								this.$nextTick(() => {

        ...
        <div v-if="value.category == 'INDEX'||value.category == 'INDEX-T'" class="mb-20">
            ...
            <div v-if="value.head.type == 'searchAndBanner'||value.head.type == 'searchAndTab'" class="bg-f9 rounded-3 p-20 mb-20">
				...
				<div class="flex-start text-nowrap mt-15 mb-15">
					...
					<el-input class="flex-1" v-model="value.head.title.name" placeholder="请输入内容" size="mini" clearable maxlength="10" show-word-limit></el-input>
				</div>
				<div class="flex-start text-nowrap mb-15" v-if="value.head.title.name">
					<span class="pr-15">&emsp;字体大小：</span>
					<el-slider
						class="flex-1"
					    v-model="value.head.title.size"
					    input-size="mini"
					    :min="15"
					    :max="30"
					></el-slider>
				...
				<div class="flex-start mb-15" v-if="!showBanner">
                ...
                <div class="flex-col-start mb-15 text-nowrap">
                    <div class="flex-start mb-15">
                        ...
                        <div class="text-wrap">
                            <el-radio v-model="value.head.locationAndLogo && value.head.locationAndLogo.flag" :label="1">
                                定位
                            </el-radio>
                            <el-radio v-model="value.head.locationAndLogo && value.head.locationAndLogo.flag" :label="2">Logo</el-radio>
                            <el-radio v-model="value.head.locationAndLogo && value.head.locationAndLogo.flag" :label="0">
                                无
                            </el-radio>
                        </div>
                    </div>
					...
					<template v-if="value.head.data.searchAndBannerData.showBanner">
						...
						<div class="d-flex a-start text-nowrap">
							...
							<div class="text-wrap">
								...
								<el-radio v-model="value.head.data.searchAndBannerData.banner.indicatorDots" class="mb-20" :label="'dot'">圆点</el-radio>
								<el-radio v-model="value.head.data.searchAndBannerData.banner.indicatorDots" class="mb-20" :label="'round'">
									柱状
								</el-radio>
								<el-radio v-model="value.head.data.searchAndBannerData.banner.indicatorDots" class="mb-20" :label="'nav'">导航</el-radio>
								<el-radio v-model="value.head.data.searchAndBannerData.banner.indicatorDots" class="mb-20" :label="'indexes'">
									数字
								</el-radio>
								<el-radio v-model="value.head.data.searchAndBannerData.banner.indicatorDots" class="mb-20" :label="'none'">关闭</el-radio>
							</div>
						</div>
						<div
                                ...
                                <div class="w-100">
                                    ...
                                    >
										...
										<div class="flex-1 overflow-h">
											<dialogUrl ref="dialog" :key="index" v-model="value.head.data.searchAndBannerData.banner.imgList[index]" :parmas="value" />
										</div>
                                    </div>
                                    <div
			...
			<div v-if="value.head.type == 'searchAndTab'" class="bg-f9 rounded-3 p-20 mb-20">
				...
				<div class="bg-f9 rounded-3 pt-20 d-flex flex-wrap">
					<div class="w-100">
						...
						<div class="overflow-h transition-all05" v-if="value.head.data.searchAndTabData.tabsShow">
							<div class="flex-start text-nowrap mb-15">
								<span class="pr-15">&emsp;&emsp;背景颜色：</span>
								<el-radio v-model="value.head.data.searchAndTabData.tabBgThemeColor" :label="false">
									<span class="mr-10">自定义</span>
									<el-color-picker v-model="value.head.data.searchAndTabData.tabBgColor" class="v-middle" show-alpha></el-color-picker>
									<el-tooltip
										class="ml-10"
										effect="dark"
			...
			<div v-if="value.head.type == 'searchAndTab'&&value.head.data.searchAndTabData.tabsShow">
				...
				<el-form ref="ruleForm1" :model="value.head.data.searchAndTabData" label-width="80px">
					...
					<el-tabs class="cusTabs" v-model="currentUUid" type="card" editable @edit="handleTabsEdit" @tab-click="tabClick">
						<el-tab-pane v-for="(item, index) in value.head.data.searchAndTabData.tabList" :key="item.uuid" class="p-10" :label="item.title" :name="item.uuid">
							<div>
								<div class="mb-20 w-100">
									<div class="flex-start text-nowrap mb-15">
										<!-- <span class="pr-15">&emsp;&emsp;标题：</span> -->
										<div class="flex-1">
											<el-form-item :prop="'tabList.' + index + '.title'" :rules="{ required: true, message: '标题不能为空', trigger: ['blur'] }" label="标题：">
												<el-input
													v-model="item.title"
													placeholder="请输入内容"
													size="mini"
									...
									<div v-if="item.type != 'homePage'" class="flex-start text-nowrap mb-15">
										...
										<el-form-item :prop="'tabList.' + index + '.url.id'" :rules="{ required: true, message: '海报不能为空', trigger: 'blur' }" label="海报：">
										<!-- , validator: (rule, value, callback)=>{checkKey(rule, value, callback, scope.$index)}, trigger: 'blur' -->
											<dialogUrl ref="dialog" :key="index" v-model="value.head.data.searchAndTabData.tabList[index]" :parmas="value"  :tab="['海报页']" @input="changeData(index)" />
										</el-form-item>
									</div>
									<!-- 首页装修 -->
									<div v-if="item.type == 'homePage'" class="flex-start text-nowrap mb-15">
										...
										<!-- , validator: (rule, value, callback)=>{checkKey(rule, value, callback, scope.$index)}, trigger: 'blur' -->
											...
											<el-tooltip
												effect="dark"
												content="备注:标签栏第一个tab默认为搜索+轮播图编辑的页面"
												placement="top-start"
											>
...
export default {
	...
	computed: {
		...
		showBanner(){
			let bool = this.value.head && this.value.head.type == 'searchAndBanner' && this.value.head.data.searchAndBannerData.showBanner&&this.value.head.data.searchAndBannerData.banner.imgList.length
			...
			if(
			  ...
			  (this.value.head.type == 'searchAndBanner' &&
			   ...
			   this.value.head.type == 'searchAndTab')
			    ...
			    this.$refs.ruleForm && this.$refs.ruleForm.validate((valid, valobj) => {
			    ...
			    this.$refs.ruleForm1 && this.$refs.ruleForm1.validate((valid, valobj) => {
			        for (var key in valobj) {

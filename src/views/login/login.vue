...
<template>
  <div class="login-wrapper">
    <div class="login-container bg-fff border-box p-30 overflow-h rounded-10">
        <div class="w-200p h-60p" v-loading="!systemConfigFinally">
            ...
            />
            <img class="w-100 h-100" src="@/assets/images/login__logo.png" alt="" v-else-if="systemConfigFinally" />
            <div class="w-100 h-100" v-else></div>
        </div>
        <div class="mt-10 flex-between login-main">
            ...
            <div class="form-wrapper border-box flex-1 ml-10 p-20 h-100">
                ...
                <el-form ref="loginForm" size="small" :model="loginForm" :rules="loginRules" class="login-form mt-30" auto-complete="on" label-position="left">
                    <el-form-item prop="principal"  style="margin-bottom:20px;">
                        <el-input ref="username" v-model="loginForm.principal" placeholder="请输入用户名" name="username" type="text" tabindex="1" autocomplete="on" size="large" >
                    ...
                    <el-form-item prop="credentials" style="margin-bottom:50px;">
                        ...
                        >
                            ...
                            <i slot="suffix" class="el-input__icon iconfont cursor-pointer" @click="showPwd" :class="[passwordType==='password' ? 'icon-password-hidden' :'icon-password-shows']"></i>
                        </el-input>
                    </el-form-item>
                    <el-form-item style="margin-bottom:0;">
                        <el-button :disabled="disabled" :loading="loading" type="primary" class="w-100" @click.native.prevent="handleLogin" size="large" style="height:45px;">登录</el-button>
                    </el-form-item>
                </el-form> 
    ...
    <footer v-if="systemConfigTemp" class="font-14 position-absolute text-999"  style="left: 50%; bottom:40px; transform: translateX(-50%); height:20px; line-height:20px;">Copyright © {{systemConfigTemp&&systemConfigTemp.domainName?systemConfigTemp.domainName:'朗尊电商'}}版权所有 {{systemConfigTemp&&systemConfigTemp.icpInfo?systemConfigTemp.icpInfo:''}}</footer>
  </div>
</template>

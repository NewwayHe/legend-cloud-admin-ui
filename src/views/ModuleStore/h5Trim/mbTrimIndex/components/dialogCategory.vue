<template>
    <el-dialog title="选择模板" :visible.sync="dialogVisible" width="820px" custom-class="dialog-form" v-loading="tableListLoading">
        <!-- 查询 -->
        <div class="search">
        ...
        <el-row v-if="tableList && tableList.length" class="d-flex flex-wrap mt-10 flex-1 px-20" :gutter="20" style="flex-warp: warp">
            <el-col v-for="item in tableList" :key="item.id" :span="6" class="pb-20 ">
				<template v-if="item.id">
					<div class="w-100 h-0 position-relative rounded-10 overflow-h bg-eee hover-current" style="padding-bottom: 125%;" @click="editInfo(item.id)">
						<ls-image class="position-absolute" :src="item.coverPicture" :options="{ w: '100%' }" :isPreview="false"/>
					</div>
					<div class="line-clamp1">
						<div class="mt-10 d-inline-block text-white px-5 font-12 rounded-3 line-h-md mr-5" :style="{background:item.type==1?'#45be89':'#ff9b00'}">
				...
				<template v-else>
					<div class="w-100 h-0 position-relative rounded-10 overflow-h bg-eee hover-current" style="padding-bottom: 125%;" @click="editInfo('')">

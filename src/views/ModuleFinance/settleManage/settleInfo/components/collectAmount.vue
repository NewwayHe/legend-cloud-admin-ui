...
<template>
    <div class="table">
        <el-table ref="multipleTable" v-loading="tableListLoading" :data="tableList" tooltip-effect="dark" class="w-100" header-row-class-name="headerRow">
            ...
            <el-table-column label="定金金额" width="115">
                <template slot-scope="scope">
                    <div>{{ scope.row.preDepositAmount | priceFilter }}</div>
                </template>
            </el-table-column>
            <el-table-column label="尾款金额" width="115">
                <template slot-scope="scope">
                    <div>{{ scope.row.finalAmount | priceFilter }}</div>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed="right" width="100">
                <template slot-scope="scope">
                    <el-link :underline="false" type="primary">
                        <router-link :to="{ name: 'orderList', query: { orderNumber: scope.row.orderNumber } }">查看</router-link>
                    </el-link>
                </template>
            </el-table-column>
        </el-table>

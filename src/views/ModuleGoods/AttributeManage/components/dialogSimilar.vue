<template>
    <el-dialog title="类似导入" :visible.sync="dialog.visible" custom-class="dialog-form-small">
        <div>
            <el-form :model="dialogForm" size="small" label-width="98px" @submit.native.prevent @keyup.enter.native="dialogSearch">
                <el-form-item label="规格标题：">
                    <el-input v-model="dialogForm.value" placeholder="请输入">
                        <i slot="suffix" class="el-input__icon el-icon-search" @click.stop="dialogSearch" />
                    </el-input>
                </el-form-item>
            </el-form>
            <el-table
...
export default {
    ...
    props: {
        ...
        url: {
            type: String,
            default: '/product/admin/productProperty/page',
            required: true
        },
        // 规格和参数的识别参数
        attributeType: {

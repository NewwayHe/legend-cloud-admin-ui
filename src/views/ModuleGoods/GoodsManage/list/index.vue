...
<template>
	<section>
			...
			<div class="table">
				...
				>
					...
					<el-table-column prop="status" label="状态" width="100">
						<template slot-scope="scope">
							<span class="status-veto" v-if="scope.row.status == -10">未提审</span>
							<span class="status-veto" v-if="scope.row.status == 0">下架</span>
							<span class="status-pass" v-if="scope.row.status == 1">上架</span>
							<span class="status-veto" v-if="scope.row.status == 2">违规商品</span>
							<span class="status-veto" v-if="scope.row.status == -1">商家删除</span>
							<span class="status-veto" v-if="scope.row.status == -2">商家永久删除</span>
						</template>
					</el-table-column>
					<el-table-column prop="draftStatus" label="草稿状态" align="left">
                        <template slot-scope="scope">
                            ...
                            <template v-if="[-10,0,-1,1].includes(scope.row.draftStatus)">
                                <span class="status-veto" v-if="scope.row.draftStatus == -10">未发布</span>
					...
					<el-table-column label="操作" fixed="right" width="140">
						<template slot-scope="scope">
							<span class="table__action flex-start">
								<el-link v-if="scope.row.status == 1" :underline="false" type="primary" @click.stop="toUnder(scope.row)">下架</el-link>
		...
		<el-dialog :title="dialogForm.title" custom-class="dialog-form-small" :visible.sync="dialogForm.isVisible" destroy-on-close :close-on-click-modal="false">
			<el-form ref="myForm" :model="dialogForm.formData" label-width="98px" label-position="right" size="small">
				...
				<el-form-item label="处理意见：" prop="handleInfo" :rules="{ required: true, message: '请填写处理意见', trigger: 'blur' }">
					<el-input v-model="dialogForm.formData.handleInfo" type="textarea" placeholder="处理意见" maxlength="50" show-word-limit autosize/>
				</el-form-item>
			</el-form>
...
export default {
	...
	methods: {
		...
		// 更新商品状态
		updateStatus(row) {
			this.$confirm(`${row.status ? '商品下架后，用户无法在商城中查看商品。确定下架？' : '商品上架后，用户可以在商城中查看商品。确定上架？'}`, '消息确认', {
				type: 'warning'
			}).then(() => {
		...
		batchUpdate(status) {
			...
			this.$confirm(`${status ? '商品上架后，用户可以在商城中查看商品。确定上架？' : '商品下架后，用户无法在商城中查看商品。确定下架？'}`, '消息确认', {
				type: 'warning'
			})
				.then(() => {
					GoodsManage.batchUpdateStatus({

    ...
    <section>
        ...
        >
            <el-form ref="myForm" :model="dialogForm.formData" :rules="dialogForm.formRule" label-width="98px" label-position="right" size="small">
                ...
                <el-form-item key="permission" label="权限编码：" prop="permission">
                    <el-input v-model="dialogForm.formData.permission" type="textarea" :rows="2" placeholder="权限编码" maxlength="1000" />
                </el-form-item>
                <el-form-item label="顺序：" prop="sort">
                    <el-input v-model="dialogForm.formData.sort" :oninput="$inputMoney" :maxlength="5" placeholder="顺序" />
                </el-form-item>
                <template v-if="dialogForm.formData.type == 0">
...
export default {
    ...
    methods: {
        // 获取整个菜单树
        getMenuTree() {
            menuAuthManage.getMenuSelect().then((res) => {
                if (res.code === 1) {
        ...
        getMenuList(params) {
            ...
            menuAuthManage.getMenuList({ params, ...this.searchFilters }).then((res) => {
                if (res.code === 1) {
        ...
        deleteMemu(row) {
            this.$confirm('确认删除该数据吗?', '提示', {
                ...
                .then((res) => {
                    menuAuthManage.deleteMenu(row.id).then((res) => {
                        if (res.code === 1) {
        ...
        saveSubmit(formName) {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    menuAuthManage[this.dialogForm.isEdit ? 'editMenu' : 'saveMenu'](this.dialogForm.formData)
                        .then((res) => {
                            if (res.code === 1) {

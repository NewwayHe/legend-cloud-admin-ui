...
const importFilters = (Vue) => {
	...
	// useSymbol 是否需要拼接 '￥'
	Vue.filter('priceFilter', (val, useSymbol = true) => {
		if (val && String(val).includes('.')) {
			...
			return (useSymbol ? '￥': '') + text.substring(0, text.indexOf(".") + 3); // 只取小数点后两位，后面的全部切割掉(不进行四舍五入)
		} else {
			if (!val) val = 0;
			return (useSymbol ? '￥': '') + Number(val).toFixed(2);
		}
	});
	...
	Vue.filter('priceComma',(money, prefix = '')=> {
        ...
        function comma(integral) {
            ...
            return  comma(integral.slice(0,-3)) + (','+integral.slice(-3))
        }
        return prefix + money
    })
	// 手机号****
	Vue.filter('phoneFilters', function(val) {
		if (!val) return '-'
		if (val.length > 7) {
